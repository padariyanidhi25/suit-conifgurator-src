...
const FabComp = () => {
  ...
  const handleConfirmClick = () => {
    if (selectedFabric) {
      console.log(`Confirmed Fabric - Name: ${selectedFabric.name}, Price: ${selectedFabricPrice}`);
    }
  ...
  const getLatestFabrics = (items) => {
    const latestFabrics = {};
    items.forEach(item => {
      if (!latestFabrics[item.type] || new Date(item.dateAdded) > new Date(latestFabrics[item.type].dateAdded)) {
  ...
  const latestFabricItems = getLatestFabrics(userData.filter(item => item.category === 'Fabric' && item.isActive));

  const filteredItems = selectedCategory === 'All Fabrics'
    ? userData.filter(item => item.category === 'Fabric' && item.isActive)
    : userData.filter(item => item.type === selectedCategory && item.category === 'Fabric' && item.isActive);
  ...
  return (
    <>
      ...
      >
        ...
        />
        <div>
          <p className="font-semibold xs:text-[1rem] xs:ml-[-4vw] xs:w-[20vw] xs:text-nowrap xs:overflow-hidden">All Fabrics</p>
        </div>
      </div>
      
      {/* Fabric items */}
      {!showDropdown && latestFabricItems.map((item, index) => (
        <div 
        ...
        >
          ...
          />
          <div>
            <p className="font-semibold xs:text-[1rem] xs:ml-[-4vw] xs:w-[20vw] xs:text-nowrap xs:overflow-hidden">{item.type}</p>
          </div>
        </div>
      ))}
